...
void tambahData(Barang daftarBarang[], int& jumlahData) {
    ...
    cin.getline(baru.nama, MAX_NAMA_LENGTH + 1); 
    if (strlen(baru.nama) > MAX_NAMA_LENGTH) {
        cout << "Panjang nama barang melebihi batas maksimum." << endl;
        return;
    }

    cout << "Masukkan Jenis Barang: ";
    cin.getline(baru.jenis, MAX_JENIS_LENGTH + 1); 
    if (strlen(baru.jenis) > MAX_JENIS_LENGTH) {
